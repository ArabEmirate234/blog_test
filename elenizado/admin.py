...
class PublicationAdmin(CustomAddmin):
    ...
    fieldsets = [
        (
            "info publication",
            {"fields": ["image", "titre", "description", "categorie"]},
        ),
        ("standard", {"fields": ["status"]}),
    ]
    ...
    def image_view(self, obj):
        return mark_safe(
            "<img src='{url}' width='100px',height='50px'>".format(url=obj.image.url)
        )
...
class CommentaireAdmin(CustomAddmin):
    ...
    fieldsets = [
        (
            "info commentaire",
            {"fields": ["nom", "commentaire", "commentaires", "publication"]},
        ),
        ("standard", {"fields": ["status"]}),
    ]
...
class ReponseCommentaireAdmin(CustomAddmin):
    ...
    fieldsets = [
        (
            "info reponse commentaire",
            {"fields": ["reponse", "nom", "commentaire", "email"]},
        ),
        ("standard", {"fields": ["status"]}),
    ]
...
class CoursAdmin(CustomAddmin):
    ...
    fieldsets = [
        (
            "info cours",
            {"fields": ["titre", "niveau", "annee", "cours", "description"]},
        ),
        ("standard", {"fields": ["status"]}),
    ]
